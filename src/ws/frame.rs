...
impl DataFrame {
    ...
    pub fn read_single(reader: &mut impl Read) -> Result<Self, Error> {
        ...
        reader.read_exact(&mut buffer[..]).map_err(|e| Error::IOError(e))?;
        ...
        let payload_len = match buffer[1] & 0b0111_1111 {
            126 => {
                ...
                reader.read_exact(&mut len_buffer[6..]).map_err(|e| Error::IOError(e))?;
                u64::from_be_bytes(len_buffer)
            }

            127 => {
                let mut len_buffer = [0u8; 8];
                reader.read_exact(&mut len_buffer[..]).map_err(|e| Error::IOError(e))?;
                u64::from_be_bytes(len_buffer)
            }
        ...
        let mask = match mask_flag {
            true => {
                ...
                reader.read_exact(&mut mask_key_buffer[..]).map_err(|e| Error::IOError(e))?;
        ...
        reader.read_exact(&mut payload[..]).map_err(|e| Error::IOError(e))?;

        if let Some(mask) = mask {
            payload = payload.into_iter().enumerate().map(|b| unmask_byte(b.0, b.1, mask)).collect();
        }
